...
int main(void) {
  ...
  for (int i = 0; i < nData; i += 2*chunkSize) {
    ...
    // Queue tasks to copy of 1 chunk of data from cpuA to gpuA for stream0 and stream1 (each).
    ...
    // Queue tasks to copy of 1 chunk of data from cpuB to gpuB for stream0 and stream1 (each).
    ...
    // Queue GPU computations over the chunks in stream0 and stream1.
    ...
    // Queue tasks to copy of 1 chunk of data from gpuC to cpuC (pinned memory) for stream0 and stream1 (each).
